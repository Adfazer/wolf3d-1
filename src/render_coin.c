...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/10/18 18:31:53 by clala             #+#    #+#             */
/*   Updated: 2020/10/18 18:31:53 by clala            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	coin->i = (int)((W / wolf->player->fov) * (coin->temp_3) - ((W / 32) * (wolf->player->dist_to_canvas / coin->dist)) / 2);
	coin->flag_1 = 0;
	coin->flag_2 = 0;
	coin->flag_i = 0;
	coin->count = 0;
	while (coin->i < (W / wolf->player->fov) * (coin->temp_3) + ((W / 32) * (wolf->player->dist_to_canvas / coin->dist)) / 2)
	{
...
void	render_coin(t_wolf *wolf, SDL_Surface *surface)
{
	...
	coin.img_location.y = (H / 2) - ((W / 32) * (wolf->player->dist_to_canvas / coin.dist)) / 2;
	SDL_BlitScaled(wolf->bon->image_coin, &coin.cut_vertical_img, surface, &coin.img_location);
}